...
resource "aws_wafv2_web_acl" "block_web_acl" {
  name        = "block-web-acl"
  description = "Web ACL with IP Set rule"
  scope       = "CLOUDFRONT" # Must match the IP Set's scope
  default_action {
    allow {}
  }
  rule {
...
resource "aws_wafv2_web_acl" "alb_acl" {
  ...
  scope       = "REGIONAL"
  description = "Minimal WAF: only CloudFront header allowed - block simple SQLi and  XSS"
  ...
  rule {
    ...
    statement {
      byte_match_statement {
        search_string         = var.cloudfront_header_value
        positional_constraint = "EXACTLY"

        field_to_match {
          single_header {
            name = lower(var.cloudfront_header_name)
          }
        }
