...
RDS_SG_ID="sg-0507e776c38844279"
...
#   backend.pkr.hcl | tee >(grep -Eo 'ami-[a-z0-9]{17}' | tail -n1 > ../../modules/asg/ami_ids/backend_ami.txt)
  ...
  -var "instance_type=t4g.small" \
  -var "vpc_id=$VPC_ID" \
  -var "subnet_id=subnet-037d9201fc8501f47" \
  -var "ssh_username=ec2-user" \
  -var "db_host=lirw-dev-db.cw5c4uumw9h5.us-east-1.rds.amazonaws.com" \
  -var "db_port=3306" \
  -var "db_user=admin_3_tier" \
  -var "db_name=lirw_react_node_app" \
  -var "db_password=Asd1-CaPQ22" \
  -var "security_group_id=$PACKER_SG_ID" \
  -var "s3_ssm_cw_instance_profile_name=S3-SSM-CW-Profile" \
  -var "bucket_name=lirw-app-bucket" \
  -var "server_key_name=server_key" \
  backend.pkr.hcl | tee >(grep -Eo 'ami-[a-z0-9]{17}' | tail -n1 > ../../modules/asg/ami_ids/backend_ami.txt)
