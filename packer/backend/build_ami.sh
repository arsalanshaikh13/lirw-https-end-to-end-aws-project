...
server_key_name=$(terraform -chdir=../../terraform/permissions output -raw server_key_name)
...
PACKER_LOG=1 PACKER_LOG_PATH=packer.log packer build \
  ...
  backend.pkr.hcl | tee >(grep -Eo 'ami-[a-z0-9]{17}' | tail -n1 > ../../terraform/compute/modules/asg/ami_ids/backend_ami.txt)


echo "Backend AMI ID has been saved to ../../terraform/compute/modules/asg/ami_ids/backend_ami.txt" 
